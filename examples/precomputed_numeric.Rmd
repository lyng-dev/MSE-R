...
#fields = c("payoff")
x1=2;x2=3;x3=-1;
DT[, payoff := 1+Distance1*x1+Distance2*x2+Distance3*x3]
#DT[,payoff:=NULL]
View(DT[Match==1,])


a<-DT[Match==1,.(Market,UpStream,DownStream)]
b<-DT[Match==1,.(Market,UpStream,DownStream)]
cc<-merge(a,b,by="Market",no.dups = FALSE,allow.cartesian=TRUE,all=TRUE)
cc<-cc[UpStream.x < UpStream.y]
