...
b<-c(1,2,3,4,5);
b[1]
up1[, (payoff):= 1+dist1*b[1]+dist2*b[2]+dist3*b[3]+dist4*b[4]+dist5*b[5]]

#=============================================================
y=1
up1[, D := rowSums(.SD * c(1,2)), .SDcols = 2:2]
up1[, D := rowSums(.SD), .SDcols = 4:5]
up1[, D := list(.SD) * c(1,2), .SDcols = 4:5]

up1[, D := rowMeans(.SD), .SDcols = 4:5]

library(globe)

up1[, D := .SD %dot% c(1,2), .SDcols = 4:5]

library(matrixStats)

w<-c(1,2)
up1[, D := rowWeightedMeans(.SD, w=w), .SDcols = 4:5]

rowWeightedMeans(x, w = w)
#=============================================================

dt[ , c(cols, 'x'), with = FALSE]

cols <- c('dist1','dist2')
d1='dist1'
d2='dist2'
up1[, (payoff):= c(..dist1,..dist2)*c(x1,x2),with=FALSE]
up1[, (payoff):= ..cols * c(1,2)]
...
up1[, payoff := 1+Distance1*x1+Distance1*x2+Distance1*x3+Distance1*x4+Distance1*x5]
up1N<-up1[,.N,by=c("Market","UpStream1","UpStream2")]
up1P<-up1[,sum(payoff),by=c("Market","UpStream1","UpStream2")]

up2[, payoff := 1+Distance1*x1+Distance1*x2+Distance1*x3+Distance1*x4+Distance1*x5]
up2N<-up2[,.N,by=c("Market","UpStream1","UpStream2")]
up2P<-up2[,sum(payoff),by=c("Market","UpStream1","UpStream2")]

up3[, payoff := 1+Distance1*x1+Distance1*x2+Distance1*x3+Distance1*x4+Distance1*x5]
up3N<-up3[,.N,by=c("Market","UpStream1","UpStream2")]
up3P<-up3[,sum(payoff),by=c("Market","UpStream1","UpStream2")]

up4[, payoff := 1+Distance1*x1+Distance1*x2+Distance1*x3+Distance1*x4+Distance1*x5]
